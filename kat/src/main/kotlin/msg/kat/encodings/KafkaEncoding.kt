...
sealed interface KafkaEncoding<T> {
  fun toProducerRecord(
    topic: String,
    data: T,
  ): ProducerRecord<ByteArray, ByteArray>

  fun fromConsumerRecord(
  ...
  ) : KafkaEncoding<ByteArray> {
    override fun toProducerRecord(
      topic: String,
      data: ByteArray,
    ) = ProducerRecord(topic, RecordKeys.randomKey(), data)
  ...
  abstract class OfStringsRepresentingBinary :
    ...
    final override fun toProducerRecord(
      topic: String,
      data: ByteArray,
    ) = ProducerRecord(topic, RecordKeys.randomKey(), data)

    final override fun fromConsumerRecord(
      consumerRecord: ConsumerRecord<ByteArray, ByteArray>,
      schema: String,
    ) = consumerRecord.value()!!
