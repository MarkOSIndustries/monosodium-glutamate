...
class Consume : KafkaTopicDataCommand() {
  ...
  private val schema by option(
    "--schema",
    "-s",
    help = "the schema name to embed in output records. Only works with encoding msg.TypedKafkaRecord",
    metavar = "uses topic name by default",
  )
  ...
  private val isolation by option("--isolation", "-i", help = "the isolation level to read with")
    .choice(
      IsolationLevel.entries.associate {
        it.toString().lowercase(Locale.ROOT) to
          it
      },
