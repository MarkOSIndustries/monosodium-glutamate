...
private fun MockConsumer<String, String>.initTopic(
  topic: String,
  partitions: Int,
  recordsPerPartition: Long,
  ...
  this.updatePartitions(
    topic,
    topicPartitions.map {
      PartitionInfo(it.topic(), it.partition(), Node.noNode(), arrayOf(Node.noNode()), arrayOf(Node.noNode()))
    },
  )
