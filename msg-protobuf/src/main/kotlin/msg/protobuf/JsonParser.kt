...
class JsonParser(
      ...
      .setField(
        ...
        if (value is JSONObject) {
          ...
          when (valueFieldDescriptor.type!!) {
            ...
            Type.SFIXED64,
            Type.UINT64,
            -> if (value is String) value.toLong() else value as Long
            Type.INT32,
            Type.SINT32,
            Type.FIXED32,
            Type.SFIXED32,
            Type.UINT32,
            -> if (value is String) value.toInt() else value as Int
  ...
  private fun readAny(
    ...
    return if (descriptor != null) {
      return com.google.protobuf.Any
        .newBuilder()
        .setTypeUrl(typeUrl)
        .setValue(readMessage(value, descriptor).toByteString())
        .build()
    } else {
  ...
  private fun readRepeatedField(
    ...
    when (field.type!!) {
      ...
      Type.SFIXED64,
      Type.UINT64,
      -> jsonArray.indices.map { i -> jsonArray.getLong(i) }
      Type.INT32,
      Type.SINT32,
      Type.FIXED32,
      Type.SFIXED32,
      Type.UINT32,
      -> jsonArray.indices.map { i -> jsonArray.getInteger(i) }
      Type.BOOL -> jsonArray.indices.map { i -> jsonArray.getBoolean(i) }
      Type.STRING -> jsonArray.indices.map { i -> jsonArray.getString(i) }
      Type.GROUP -> TODO()
      Type.MESSAGE ->
        specialTypes[field.messageType.fullName]?.let { jsonArray.indices.map { i -> it(jsonArray[i], field) } }
  ...
  private fun readMapField(
    ...
    return jsonObject.keys.map { key ->
      DynamicMessage
        .newBuilder(
          field.messageType,
        ).setField(keyField, readKeyField(key, keyField))
        .setField(valueField, readField(jsonObject, key, valueField))
        .build()
    }
  }

  private fun readKeyField(
    key: String,
    field: Descriptors.FieldDescriptor,
  ): Any =
    when (field.type!!) {
      ...
      Type.ENUM ->
        field.enumType.values.firstOrNull { it.name == key }
  ...
  fun readField(
    ...
    when (field.type!!) {
      ...
      Type.SFIXED64,
      Type.UINT64,
      -> parentJsonObject.getLong(key)
      Type.INT32,
      Type.SINT32,
      Type.FIXED32,
      Type.SFIXED32,
      Type.UINT32,
      -> parentJsonObject.getInteger(key)
      Type.BOOL -> parentJsonObject.getBoolean(key)
      Type.STRING -> parentJsonObject.getString(key)
      Type.GROUP -> TODO()
      Type.MESSAGE ->
        specialTypes[field.messageType.fullName]?.let { it(parentJsonObject[key]!!, field) }
      ...
      Type.ENUM ->
        parentJsonObject.getString(key).let { asString ->
          field.enumType.values.firstOrNull { it.name == asString }
            ?: asString.toIntOrNull()?.let { asInt -> field.enumType.values.firstOrNull { it.number == asInt } }
            ?: throw InvalidProtocolBufferException("$asString is not a valid ${field.enumType.fullName} value")
        }
