...
abstract class ProfileAwareCommand(
  val args: Array<String>,
  name: String? = null,
) : CliktCommand(name) {
  val profileDirDefault = System.getProperty("user.home") + "/.msg"
  val profileDirOption =
    option(
      "--profile-dir",
      help = "the directory MSG profiles live in",
      envvar = "MSG_PROFILE_DIR",
    ).default(profileDirDefault)
  ...
  companion object {
    private fun valueSource(
      args: Array<String>,
      cmd: ProfileAwareCommand,
    ): ValueSource {
      val parse = CommandLineParser.parse(cmd, args.asList())

      val profileDir: String =
        parse.invocation.optionInvocations[cmd.profileDirOption]
          ?.firstOrNull()
          ?.values
          ?.firstOrNull()
      ...
      val profile: String =
        parse.invocation.optionInvocations[cmd.profileOption]
          ?.firstOrNull()
          ?.values
          ?.firstOrNull()
