...
class Services : ProtobufCommand() {
  ...
  private val query by argument(
    help = "Filter the service list by fully qualified name (case-insensitive, supports * wildcards)",
  ).default("*")
  ...
  private val fields by option(
    "--methods",
    "-m",
    help = "Include a description of the services' methods in the output",
  ).flag(default = false)
        ...
        if (fields) {
          for (method in serviceDescriptor.methods) {
            ...
            terminal.println(
              ...
              )} => ${Colours.metadata(serverStreaming)}${Colours.type(method.outputType.fullName)}",
            )
          }
        }
      }
