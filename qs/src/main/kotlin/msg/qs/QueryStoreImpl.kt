...
class QueryStoreImpl(
  private val rocksDBManager: RocksDBManager,
) : QueryStoreGrpc.QueryStoreImplBase() {
  override fun get(
    request: MSG.GetRequest,
    responseObserver: StreamObserver<MSG.GetResponse>,
  ) {
    try {
      val value =
        rocksDBManager.rocksdb.get(
          rocksDBManager.columnFamilyHandleList[RocksDBManager.defaultColumnFamilyIndex],
        ...
        responseObserver.onNext(
          MSG.GetResponse
            ...
            .setValue(
              Any
                .newBuilder()
                .setTypeUrl("type/${request.schema}")
  ...
  override fun getKeyCounts(
    request: MSG.GetKeyCountsRequest,
    responseObserver: StreamObserver<MSG.GetKeyCountsResponse>,
  ) {
    try {
      ...
      (responseObserver as ServerCallStreamObserver<MSG.GetKeyCountsResponse>).sendWithBackpressure(
        iterator,
        rocksIterator,
      ) { (key, value) ->
        MSG.GetKeyCountsResponse
          .newBuilder()
          .setKey(ByteString.copyFrom(key))
          .setCount(longFromByteArray(value))
          .build()
      }
    } catch (t: Throwable) {
      t.printStackTrace()
  ...
  override fun put(
    request: MSG.PutRequest,
    responseObserver: StreamObserver<MSG.PutResponse>,
  ) {
    try {
      rocksDBManager.rocksdb.put(
        rocksDBManager.columnFamilyHandleList[RocksDBManager.defaultColumnFamilyIndex],
        request.key.toByteArray(),
        request.value.toByteArray(),
      )
      rocksDBManager.rocksdb.merge(
        rocksDBManager.columnFamilyHandleList.get(RocksDBManager.countsColumnFamilyIndex),
        request.key.toByteArray(),
        longToByteArray(1L),
      )
  ...
  override fun delete(
    request: MSG.DeleteRequest,
    responseObserver: StreamObserver<MSG.DeleteResponse>,
  ) {
    try {
      rocksDBManager.rocksdb.delete(
        rocksDBManager.columnFamilyHandleList[RocksDBManager.defaultColumnFamilyIndex],
        request.key.toByteArray(),
      )
